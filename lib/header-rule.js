...
function headerMatches(s, normHeader, opts) {
    console.log('XXX headerMatches: %j', s);
    if (s === normHeader) {
        return true;
...
function create(context) {
    ...
    return {
        Program: (node) => {
            ...
            let shebang = null;
            if (leadingComments.length > 0 && leadingComments[0].type === 'Shebang') {
            ...
            if (leadingComments.length === 0) {
                context.report({
                    ...
                    fix(fixer) {
                        const start = shebang ? shebang.end + eol.length : 0;
                        const end = node.start
                        return fixer.replaceTextRange([start,end], eolReplace(normHeader + '\n\n', eol));
                    },
                });
                return;
            }
            ...
            if (leadingComments[0].type === 'Block') {
                ...
                considerComment = '/*' + eolNormalize(leadingComments[0].value) + '*/'
            } else {
                ...
                for (const c of leadingComments.slice(1)) {
                    ...
                    if (c.start > considerRange[1] + 1) {
                        break;
                    }
                    considerRange[1] = c.end;
                    lines.push('//' + c.value);
                }
            ...
            if (!looksLikeLicenseComment(considerComment)) {
                context.report({
                    ...
                    fix(fixer) {
                        const start = shebang ? shebang.end + eol.length : 0;
                        const end = leadingComments[0].start;
                        return fixer.replaceTextRange([start,end], eolReplace(normHeader + '\n\n', eol));
                    },
                });
                return;
            ...
            if (!headerMatches(considerComment, normHeader, opts)) {
                context.report({
                    ...
                    fix(fixer) {
                        return fixer.replaceTextRange(considerRange, eolReplace(normHeader, eol));
                    },
                });
                return;
            }
