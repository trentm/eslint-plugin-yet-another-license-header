...
function create(context) {
    ...
    return {
        Program: (node) => {
            ...
            let leadingComments;
            if (node.body[0]) { // first statement
                leadingComments = context.sourceCode.getCommentsBefore(node.body[0]);
            } else {
                leadingComments = context.sourceCode.getAllComments();
            }
            ...
            if (
                ...
                shebang = leadingComments[0];
                // Do not modify in-place, because `.getAllComments()` returns
                // the actual AST.
                leadingComments = leadingComments.slice(1);
            }
